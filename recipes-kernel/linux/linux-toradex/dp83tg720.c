...
static int DP83TG720_cable_test_start(struct phy_device *phydev){
	...
	if(DP83TG720_read_master_slave(phydev))
	{
		...
		msleep(100);
		pr_err("PHY is set as Master.\n");
	} else
	{
		pr_err("PHY is set as Slave. \n)");
	}
...
static int DP83TG720_cable_test_report_trans(u32 result) {
	...
	switch(TDR_STATE){
		case 3:
			pr_err("Short detected. \n");
			break;
		case 5:
			pr_err("Noise detected. \n");
			break;
		case 6:
			pr_err("Open detected. \n");
			break;
		case 7:
			pr_err("No fault detected. Cable OK \n");
			break;
		case 8:
			pr_err("Test in progress; initial value with TDR ON \n");
			break;
		case 13:
			pr_err("Test not possible (for example, noise, active link) \n");
			break;
		default :
